...
INGREDIENTS = {
    ...
    "oysters_eastern_raw": {"fdc_id": 171978, "zh": "生蚝(生)", "desc": "Mollusks, oyster, eastern, wild, raw"},
}
...
def main():
    ...
    target_ids = {info["fdc_id"] for info in INGREDIENTS.values()}
    search_strings = {f',"{fid}",' for fid in target_ids}
    ...
    with open(FOOD_NUTRIENT_CSV, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line_count % 5000000 == 0:
                print(f"  Scanned {line_count/1e6:.0f}M lines, found {found_count} records...")
    ...
    print(f"\n  Scanned {line_count/1e6:.0f}M lines, found {found_count} nutrient records")
    ...
    for key, info in INGREDIENTS.items():
        ...
        print(f"  {info['zh']}: {len(nutrients)} nutrients")

    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump(database, f, ensure_ascii=False, indent=2)

    total = sum(v["nutrient_count"] for v in database.values())
    print(f"\nDone! {len(database)} ingredients, {total} total nutrient records")
